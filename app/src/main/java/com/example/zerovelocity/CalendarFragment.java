...
public class CalendarFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        tvSelectedDate.setText("Events for: " +
                DATE_FORMAT.format(new Date(selectedDate)));
    ...
    private void listenToEvents(String currentUid) {
        ...
        eventsRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot child : snapshot.getChildren()) {
                    ...
                    boolean isCreator = currentUid.equals(createdBy);
                    boolean isInvited =
                            child.child("invites").child(currentUid).exists();

                    if ((isCreator || isInvited) && title != null && date != null) {
    ...
    private void showAddEventDialog() {
        ...
        btnStartDate.setOnClickListener(v ->
                pickDateTime(startMillis, chosen -> {
                    startMillis[0] = chosen;
                    tvStartChosen.setText("Start: " +
                            fmt.format(new Date(chosen)));
                }));

        btnEndDate.setOnClickListener(v ->
                pickDateTime(endMillis, chosen -> {
                    endMillis[0] = chosen;
                    tvEndChosen.setText("End: " +
                            fmt.format(new Date(chosen)));
                }));

        builder.setTitle("Add Event on " +
                        DATE_FORMAT.format(new Date(selectedDate)))
                .setView(dialogView)
                .setPositiveButton("Save", (dialog, which) -> {
                    ...
                    saveEvent(eventTitle, selectedDate,
                            startMillis[0], endMillis[0], currentUid);
                })
    ...
    private void saveEvent(String title,
        ...
        DatabaseReference dbRef =
                FirebaseRefs.root().child("Events");
        ...
        Map<String, Boolean> invites = new HashMap<>();
        invites.put(currentUid, true); // creator is always included
